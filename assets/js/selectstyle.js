...
(function($){
	$.fn.selectstyle = function(option){
		...
		this.each(function(){
			...
			html = 
			'<div class="selectstyle ss_dib '+setting.theme+'" style="width:'+parseInt(setting.width)+'px;">'+
				'<div id="select_style" class="ss_button" style="">'+
					'<div class="ss_dib ss_text" id="select_style_text" style="margin-right:15px;position:relative;">'+placeholder+'</div>'+
					'<div class="ss_dib ss_image"></div>'+
				'</div>';	
			if(search == "true"){
				...
				html += '<ul id="select_style_ul" sid="'+id+'" class="ss_ulsearch" style="max-height:'+setting.height+'px;"><div class="search" id="ss_search"><input type="text" placeholder="Search"></div><ul style="max-height:'+(parseInt(setting.height) - 53)+'px;" class="ss_ul">'+html_op+'</ul></ul>';
			}
			else{
