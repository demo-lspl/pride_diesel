...
function arrayToCSV($inputArray)
{
	...
	public function generate_trans_invoice($cid){
		...
		if(count((array)$companyPricingType)>0){
			...
			$pricingTypeGet = $companyPricingType->usa_pricing;
			$pricingType = $companyPricingType->usa_pricing;
			//$pricingType = $this->db->select($pricingTypeGet)->get('pricelist_edit_us')->row();
...
/* 		foreach($pricelistDecoded as $key=>$pricelistDecodedRow){
		...
		$priceListUSD = $this->db->where('id', 1)->get('pricelist_us')->row();
		$companyTypeVal = strtolower($getCompanyType->company_type);
		if($priceListUSD->$pricingType){
			$makeDefdName = 'defd_'.$companyTypeVal;
			$decodePricelistUSA = json_decode($priceListUSD->$pricingType);
			$DEFD = 'DEFD';
			foreach($decodePricelistUSA as $decodePricelistCARows){
				
				if(array_key_exists('DEFD',$decodePricelistCARows)){
					$defdPriceByCompType = $decodePricelistCARows->$DEFD[0]->$makeDefdName[0];
				}
			}
			...
			if($productName == 'DEFD'){
				//$companyTypePrice = $defdPriceByCompType;
				$priceAfterQty = floatval($defdPriceByCompType) * $jsonDecodeQuantity[$jsonDecodeIncrement];
			}else{
				$priceAfterQty = floatval($companyTypePrice) * $jsonDecodeQuantity[$jsonDecodeIncrement];
			}
		if($productName == 'DEFD'){$RetailCost = $defdPriceByCompType;}else{$RetailCost = $companyTypePrice;}			
		...
		$content .= '
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$RetailCost.'</td>
    ...
    background-color: #ededed;
		...
		$totalTransAmount=0; $grandTotalAmountFinal=0;
		...
		$companyTypeVal = strtolower($getCompanyType->company_type);
		if($priceListCAD->$pricingType){
			$makeDefdName = 'defd_'.$companyTypeVal;
			$decodePricelistCA = json_decode($priceListCAD->$pricingType);
			$DEFD = 'DEFD';
			foreach($decodePricelistCA as $decodePricelistCARows){
		...
		//die;
		$productPriceList = $this->db->get('pricelist_edit_ca')->row();
		$pricelistDecoded = json_decode($productPriceList->$pricingType);
		foreach($pricelistDecoded as $key=>$pricelistDecodedRow){
		...
		$amountTotal = $priceAfterQty + $includeGST + $includePST + $includeQST;
		//$subTotal = $amtAfterTax + $includeGST + $includePST + $includeQST + $companyTypePrice;
		$subTotal = $priceAfterQty + $includeGST + $includePST + $includeQST;
		...
		$content .= '
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$usdCardRows->gas_station_id.'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$jsonDecodeProduct[$jsonDecodeIncrement].'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">-</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$totalSavings.'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$jsonDecodeQuantity[$jsonDecodeIncrement].'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
		...
		$grandTotalAmountFinal += number_format($subTotal, 2);
			...
			/*trans_data field*/
			$transJsonArrayObject = (array($usdCardRows->card_number => array('product_name'=>$jsonDecodeProduct[$jsonDecodeIncrement], 'quantity'=>$jsonDecodeQuantity[$jsonDecodeIncrement], 'unit_price'=>$companyTypePrice, 'GST'=>$gstByCard, 'PST'=>$pstByCard, 'QST'=>$qstByCard, 'amountwithouttax' => number_format($priceAfterQty, 2), 'grand_total'=> $subTotal)));
			$transationDetails[$usdCount] = $transJsonArrayObject;
			/*invoice_data field*/
			$jsonArrayObject = (array('card_number' =>$usdCardRows->card_number,'transaction_date' => $usdCardRows->transaction_date, 'transaction_id' => $usdCardRows->transaction_id));
			$arr[$usdCount] = $jsonArrayObject;
    ...
    <table class="table" border="" style="border: 1px solid #dcdcdc; font-size:8px; margin:20px auto; border-collapse:collapse;" cellspacing="0" cellpadding="3">  
		 ...
		 <tr>
			...
			<td>'.number_format($grandQST, 2).'</td>
			<td>'.number_format($grandPST, 2).'</td>
			<td>'.number_format($totalDiscount, 2).'</td>
			<td>'.floor($totalTransAmount * 100)/100 .'</td>
			<td>CAD</td>
		</tr>
