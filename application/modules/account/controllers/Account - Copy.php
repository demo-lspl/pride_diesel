...
echo "Imported";
	...
	public function generate_trans_invoice($cid){
		...
		foreach($this->data['cardsTransData'] as $cardDetails){
		  ...
		  $cardDetail[$count]['category_'.$count] = $cardDetails->category;
		  $cardDetail[$count]['transaction_date_'.$count] = $cardDetails->transaction_date;
		  $cardDetail[$count]['quantity_'.$count] = $cardDetails->quantity;
		  $cardDetail[$count]['amount_'.$count] = $cardDetails->amount;
		  $cardDetail[$count]['unit_price_'.$count] = $cardDetails->unit_price;
		  ...
		  $data['gst_total'] = $this->input->post('gst_total');
		  $data['pst_total'] = $this->input->post('pst_total');
		...
		$obj_pdf->AddPage();
			...
			//$content .= $taxapplicable;
				$content .='<table border="0" cellspacing="0" cellpadding="5">
							<tr>
								<td width="50%"><img src="'.$image.'" alt="company_logo" width="200" height="100" border="0"/></td>
								<td width="50%">
								<table border="1" cellspacing="0" cellpadding="3">
									<tr><td colspan="3" align="center">INVOICE</td></tr>
									<tr><th align="center">DATE</th><th align="center">INVOICE</th><th align="center">PAGE</th></tr>
			  ...
			  $content .= '<table class="table" border="" style="border: 1px solid #1e1e1e;" cellspacing="0" cellpadding="3" nobr="true">  
				   <tr >  
						<th width="23%" style="font-size:9px;text-align:center;border-bottom: 1px solid #1e1e1e;">SITE</th>  
						<th width="18%" style="font-size:9px;border-bottom: 1px solid #1e1e1e;">GAS STATION</th>  
						<th width="15%" style="font-size:9px;border-bottom: 1px solid #1e1e1e;">DATE & TIME</th>  
						<th width="10%" style="font-size:9px;border-bottom: 1px solid #1e1e1e;">QTY</th>   
						<th width="12%" style="font-size:9px;border-bottom: 1px solid #1e1e1e;">FUEL TAXES</th>   
						<th width="12%" style="font-size:9px;border-bottom: 1px solid #1e1e1e;">PRICE UNIT</th>   
			...
			for($i = 0 ;$i<5;$i++){
				if($cardDetail[$k]['quantity_'.$k] != ''){				
					...
					<tr><td>PRODUCT: ".$cardDetail[$k]['category_'.$k]." </td><td>".$cardDetail[$k]['gas_station_name_'.$k]."</td><td>".$cardDetail[$k]['transaction_date_'.$k]."</td><td>".$cardDetail[$k]['quantity_'.$k]."</td><td>".number_format($cardDetail[$k]['ppftotal_'.$k], 4)."</td><td>".$cardDetail[$k]['unit_price_'.$k]."</td><td>".number_format($cardDetail[$k]['grandamount_'.$k], 2)."</td></tr>
					";
			...
			if($j == $after_divide-1){			
				...
				<tr>
					...
					<table border="1" cellpadding="3" nobr="true"> 
						<tr><td>SUB-TOTAL</td><td>'.$this->input->post('sub_total').'</td></tr>
						<tr><td>G.S.T.</td><td>'.$this->input->post('gst_total').'</td></tr>
						<tr><td>P.S.T.</td><td>'.$this->input->post('pst_total').'</td></tr>
						<tr><td >TOTAL</td><td style="border-left: 1px solid transparent;">'.$this->input->post('grand_total').'</td></tr>
					</table>
					</td>
				</tr>	
				</table>
