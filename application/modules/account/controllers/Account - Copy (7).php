...
function arrayToCSV($inputArray)
{
	...
	public function generate_trans_invoice($cid){
		...
		$obj_pdf->AddPage();
			...
			$totalDiscountFinal = 0; $grandTotalAmountFinal = 0;
			$totalTransactions;	
		...
		$totalTransactionsDatfile += count($jsonDecodeProduct);
		...
		$amountByEfs = $efsRetailAmount * $jsonDecodeQuantity[$jsonDecodeIncrement];
		$amountByPride = $prideDieselPrice * $jsonDecodeQuantity[$jsonDecodeIncrement];
		$totalSavings = '0';
		$discount = 0;
		if($productName != 'DEFD'){
			$totalSavings = $efsRetailAmount - $prideDieselPrice;
			$totMargin = $amountByEfs - $amountByPride;
			$discount = floor($totMargin*1000)/1000;
		}
		...
		if($usdCount % 2 == 0){
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$discount.'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
		...
		$transid = $usdCardRows->transaction_id;
		$cardNumber = $usdCardRows->card_number;
		$invoiceNum = $usdCardRows->invoice;
		$locId = $usdCardRows->gas_station_id;
		$transDateTime = date('Ymdhi', strtotime($usdCardRows->transaction_date));
		$authCode = '0000000000';
		$contractId = '0000000000';
		$issuerId = '0000000000';
		$carrierId = '0000000000';
		$supplierId = '0000000000';
		$reportedCarrier = date('Ymdhis', strtotime($usdCardRows->transaction_date));
		$handEntered = 'N';
		$overrideCard = 'N';
		...
		$dicountType = 'W';
		$POSdate = date('Ymdhi', strtotime($usdCardRows->transaction_date));
		$currencyCode = 'USD';
		...
		$datContent .= $transType.$isCredit.$settleAmount.$locType;/*transType(1), isCredit(1), settleAmount(11), locType(4)*/
		$datContent .= "       ";/*Filler(7)*/
		...
		/*TA Code Ended*/
		$lineId = '00';
		$productCat = $jsonDecodeProduct[$jsonDecodeIncrement];
		$quantity = str_pad(str_replace('.','',$jsonDecodeQuantity[$jsonDecodeIncrement]), 7, "0", STR_PAD_LEFT);
		$amount = str_pad(str_replace('.','',number_format($jsonDecodePridePrice[$jsonDecodeIncrement], 4)), 10, "0", STR_PAD_LEFT);
		$retailAmt = str_pad(str_replace('.','',$efsRetailAmount), 10, "0", STR_PAD_LEFT);
		$fuelType = '00000000';
		$serviceType = '00';
		$fuelUseType = '00';
		$ppuDiscounted = '0000000';
		$retailPPU = str_pad(str_replace('.','',$efsRetailAmount), 10, "0", STR_PAD_LEFT);
		$productNum = '000';
		$unitOdMeasure = 'G';
		...
		$grandTotalAmount += number_format($total_amount, 2);
		$totalQuantity = $jsonDecodeQuantity[$jsonDecodeIncrement];
		//$totalDiscountFinal += $totalSavings;
		$totalDiscountFinal += $discount;
		$grandTotalAmountFinal += number_format($total_amount, 2);
		
			/*trans_data field*/
			$transJsonArrayObject = (array($usdCardRows->card_number => array('product_name'=>$jsonDecodeProduct[$jsonDecodeIncrement], 'quantity'=>$jsonDecodeQuantity[$jsonDecodeIncrement], 'unit_price'=>number_format($jsonDecodePridePrice[$jsonDecodeIncrement], 4), 'amountwithouttax' => number_format($total_amount, 2))));
	...
	//}	
		...
		<p style="">QST# 1221749509TQ0001</p>';
		$gTamount = floor($grandTotalAmountFinal*100)/100;
		$totNumOfTrans = str_pad(str_replace('.','',$totalTransactionsDatfile), 10, "0", STR_PAD_LEFT);
		$totNumOfRecs = str_pad(str_replace('.','',$totalTransactionsDatfile), 10, "0", STR_PAD_LEFT);
		$totTransAmtSign = '+';
		$totTransAmt = str_pad(str_replace('.','',$gTamount), 11, "0", STR_PAD_LEFT);
		/*HT Code Started*/
		...
		/*HT Code Ended*/		
		  ...
		  $this->db->insert('transaction_invoice', $data);
		  $obj_pdf->writeHTML($content);	
		$file = fopen(APPPATH ."modules/cronjob/invoice_pdf/testing.dat","w");
		//fwrite($file,"$a\n$b$c\n$e$d$f$k$g$l$h$m\n$n$o$p$q$r$s$t\n$u$v");
