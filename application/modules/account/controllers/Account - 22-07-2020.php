...
class Account extends MY_Controller {
	...
	public function invoice(){
		...
		$this->data['allInvoices'] = $this->account_model->get_invoices($where);
        ...
        $this->data['allInvoices'] = $this->account_model->get_pagination_invoice($config['per_page'], $page, $where);		
...
echo "Imported";
	...
	public function generate_trans_invoice($cid){
		...
		foreach($this->data['cardsTransData'] as $cardDetails){
			$subTotal = 0; $gstCount = 0; $pstCount = 0; $productprice = 0;$multi_trans = 0; $fet=0;$pft=0;$pct=0;
			...
			//$decodeCardCat = json_decode($cardDetails->category);
				...
				//foreach($decodeCardCat as $cat_vals){
					...
					$taxapplicable = $this->db->where('product_name', $cardDetails->category)->get('products')->row();								
					...
					$productName = json_decode($cardDetails->category);
					$productPriceList = $this->db->get('pricelist')->row();
					$pricelistDecoded = json_decode($productPriceList->price_descr);
					$jsonDecodeCategory=0;
					for($decCat=0; $decCat<count($decodeCategory);$decCat++){
						//pre($decodeCategory[$jsonDecodeCategory]);
						foreach($pricelistDecoded as $key=>$pricelistDecodedRow){
								
							if(array_key_exists($decodeCategory[$jsonDecodeCategory], $pricelistDecodedRow)){
								$catProductName = $decodeCategory[$jsonDecodeCategory];
								//pre($pricelistDecodedRow->$catProductName[0]->efs_amt);
							unset($pricelistDecodedRow->$catProductName[0]->efs_amt);
							//pre($pricelistDecodedRow->$decodeCategory[$jsonDecodeCategory]);pre($pricelistDecodedRow->$decodeCategory[$decCat]);
							$companyTypePrice = $pricelistDecodedRow->$catProductName[0]->$typeOfCompany;
							}
							//echo "Yes<br/>";
						}

//pre($companyTypePrice);
					$tax3withprice = number_format($decodeUnitPrice[$jsonDecodeCategory]+$companyTypePrice[0], 4) + number_format($ppfTot, 4);
					//$amtAfterTax = $cardDetails->quantity * number_format($tax3withprice, 4);
					$amtAfterTax = $decodeQuantity[$jsonDecodeCategory] * number_format($tax3withprice, 4);
					if(empty($gst)){$gst = 0;}
					if(empty($pst)){$pst = 0;}
					...
					$cardDetail[$count]['category_'.$count] = $decodeCategory[$jsonDecodeCategory];
					$cardDetail[$count]['transaction_date_'.$count] = $cardDetails->transaction_date;
					$cardDetail[$count]['quantity_'.$count] = $decodeQuantity[$jsonDecodeCategory];
					$cardDetail[$count]['amount_'.$count] = $decodeAmount[$jsonDecodeCategory];
					$cardDetail[$count]['unit_price_'.$count] = $decodeUnitPrice[$jsonDecodeCategory];
