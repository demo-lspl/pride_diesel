...
class Account extends MY_Controller {
	...
	public function invoiced($id){
		...
		$this->data['allInvoices'] = $this->account_model->get_invoiced_transactions($where);
        ...
        $this->data['allInvoices'] = $this->account_model->get_pagination_invoiced_trans($config['per_page'], $page, $where);
	...
	public function exportTransactionByCompany($cid){
        ...
        foreach ($empInfo as $element) {
					...
					$pricingTypeCA = $get_user_type->cad_pricing;
							...
							if($element['billing_currency'] == 'CAD'){
								...
								$finalGST=0;$finalPST=0;$finalQST=0;
									$priceList = $this->db->get('pricelist_edit_ca')->row();
									$PriceDecode = json_decode($priceList->$pricingTypeCA);
									$makeDefdName = 'defd_'.$companyType;
									foreach($PriceDecode as $PriceDecodeRows){
										if(array_key_exists($pname, $PriceDecodeRows)){
											//pre($PriceDecodeRows->$pname[$i]->$companyType[$i]);
											$caPrice = $PriceDecodeRows->$pname[$i]->$companyType[$i];
											$defdPriceByCompType = $PriceDecodeRows->$pname[$i]->$makeDefdName[0];
										}
									}
									if($pname == 'DEFD'){
										$amtAfterDec = floatval($defdPriceByCompType);
									}else{
										//$priceAfterQty = floatval($amtAfterDec) * floatval($decQuantity[0]);
										$amtAfterDec = $caPrice;
									}									
									$subTotalAmt = floatval($decQuantity[0]) * $amtAfterDec;									
								foreach($getTaxRate as $taxTypeRows){

									if($taxTypeRows->tax_type == 'gst'){
										$gstRate = str_replace('%', '', $taxTypeRows->tax_rate);
										$finalGST = $subTotalAmt * $gstRate / 100;
									}
									if($taxTypeRows->tax_type == 'pst'){
										$pstRate = str_replace('%', '', $taxTypeRows->tax_rate);
										$finalPST = $subTotalAmt * $pstRate / 100;
									}
									if($taxTypeRows->tax_type == 'qst'){
										$qstRate = str_replace('%', '', $taxTypeRows->tax_rate);
										$finalQST = $subTotalAmt * $qstRate / 100;
									}				
									$totalTaxAmount = number_format($finalGST + $finalPST + $finalQST, 2);
								}								
								$amtAfterDecTax = floatval($decQuantity[0]) * $amtAfterDec + $totalTaxAmount;
							}
									...
									foreach($dailyPriceList as $dailyPriceListRows):
									...
									if(!empty($dynamicrows->$pname[$i]->$companyType[$i])){
										$usPrice = $dynamicrows->$pname[$i]->$companyType[$i];
										$defdPriceByCompType = $dynamicrows->$pname[$i]->$makeDefdName[0];
										//$amtAfterDec = $usPrice;
										$amtAfterDecTax = floatval($decQuantity[0]) * $dynamicrows->$pname[$i]->$companyType[$i];
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $amtAfterDecTax);
            $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $element['category']);
            $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $amtAfterDec);
            $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $element['quantity']);
       ...
       $objWriter = new PHPExcel_Writer_Excel2007($objPHPExcel); 
			...
			$object_writer = new PHPExcel_Writer_Excel2007($objPHPExcel);//pre($object_writer);die;
			       ...
			       header("Content-Disposition: attachment;filename=Exporttransactions.xlsx");
			         ob_end_clean();
	...
	public function export_company_trans_csv($cid){
	   ...
	   // get data 
		...
		//pre($this->db->last_query());die;
		$dailyPriceList = $this->user_model->get_dailypricelist();	/* pre($element);die; */	
		// file creation 
		$file = fopen('php://output','w');		
		$header = array("Billing Currency","Card Number","Invoice","Amount", 'Category', 'Unit Price', 'Quantity', 'GST/PST/QST', 'Gas Station Name', 'Gas Station City', 'Gas Station State', 'Transaction Date', 'Date Created'); 
		fputcsv($file, $header);
		foreach ($usersData as $key=>$element){
			...
			$cardNumberVal = $element['card_number'];
			$lineVal = 	array($element['billing_currency'], intval($cardNumberVal), $element['invoice'], $amtAfterDecTax, $element['category'], $amtAfterDec, $element['quantity'], $totalTaxAmount, $element['gas_station_name'], $element['gas_station_city'], $element['gas_station_state'], $element['transaction_date'], $element['date_created']);

			$CSVData = arrayToCSV($lineVal);	
...
function arrayToCSV($inputArray)
{
	...
	public function generate_trans_invoice($cid){
		...
		$jsonDecodePridePrice 			= json_decode($usdCardRows->pride_price);   
		...
		$efsRetailAmount = $jsonDecodeUnitPrice[$jsonDecodeIncrement];
		$prideDieselPrice = $jsonDecodePridePrice[$jsonDecodeIncrement];
		$totalSavings = $efsRetailAmount - $prideDieselPrice;
		$total_amount = $prideDieselPrice * $jsonDecodeQuantity[$jsonDecodeIncrement];
		if($usdCount % 2 == 0){
		...
		$content .= '
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$efsRetailAmount.'</td>
		...
		if($usdCount % 2 == 0){
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$prideDieselPrice.'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.number_format($prideDieselPrice, 2).'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
		...
		</tr>';$totalDiscount += $totalSavings; $grandTotalAmount += number_format($total_amount, 2);
		$totalQuantity = $jsonDecodeQuantity[$jsonDecodeIncrement];
		$totalDiscountFinal += $totalSavings;$grandTotalAmountFinal += number_format($total_amount, 2);
	...
	$this->db->group_by('transactions.card_number');
	$cardCount = $this->db->get('cards')->result();
	$grandGST=0;$grandPST=0;$grandQST=0;$grandTransAmount =0;
	foreach($cardCount as $cardRows){
	$gstByCard =0; $qstByCard=0; $pstByCard=0;  $allGSTbyCard=0;$allPSTbyCard=0;$allQSTbyCard=0;
	$this->db->select('cards.*, users.*, transactions.id as transid, transactions.*');		
	$this->db->join('users', 'users.id = cards.company_id');
	$this->db->join('transactions', 'transactions.card_number = cards.card_number');
	...
	$content .= '<h3>Transactions for card: '.$cardRows->card_number.'</h3>
      <table style="border: 1px solid #dcdcdc; border-collapse:collapse; font-size: 8px;" cellpadding="3"> 
		...
		$companyPricingType = $this->db->where('id', $cid)->get('users')->row();
		$usdCount = 1;
		$totalDiscount = 0;$grandTotalAmount = 0; $totalQuantity = 0; $gstCount=0; $pstCount=0; $qstCount=0; $qtyByCard=0;
		$totalTransAmount=0; $grandTotalAmountFinal=0;$grandTotalAmountByCard=0; 
		...
		if($getGasStation->exclude_pack_price == 1){
			...
			$this->db->select('users.fix_cost_data, users.cad_pricing');
			$this->db->from('cards');		
			$this->db->join('users', 'users.id=cards.company_id');
			$this->db->where('cards.card_number', $usdCardRows->card_number);
			$getFixPriceStatus = $this->db->get()->row();
			if(!empty($getFixPriceStatus->cad_pricing)){
		...
		$priceListCAD = $this->db->where('id', 1)->get('pricelist_ca')->row();

		$companyTypeVal = strtolower($getCompanyType->company_type);


		$priceAfterQty = $prideDieselPrice * $jsonDecodeQuantity[$jsonDecodeIncrement];
		...
		$billedPrice = 	$prideDieselPrice;
		$totalSavings = 0; $discount = 0; $efsRetailPrice = 0;
		if($jsonDecodeProduct[$jsonDecodeIncrement] != 'DEFD'){
			//$RetailCost = $efsRetailPrice;
			$efsRetailPrice = $jsonDecodeUnitPrice[$jsonDecodeIncrement];
			$retailAfterQty = $efsRetailPrice * $jsonDecodeQuantity[$jsonDecodeIncrement];
			$discount = $retailAfterQty - $priceAfterQty;
			$totalSavings = $efsRetailPrice - $prideDieselPrice;
		}
				
		//$amountTotal = $amtAfterTax + $companyTypePrice * $jsonDecodeQuantity[$jsonDecodeIncrement];
		$amountTotal = $priceAfterQty;
		//$subTotal = $amtAfterTax + $includeGST + $includePST + $includeQST + $companyTypePrice;
		$subTotal = $priceAfterQty;
		...
		$content .= '
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;">'.number_format($subTotal, 2).'</td>
		</tr>';	
		if($usdCount % 2 == 0){
			$content .= '<tr class="even-bg">';
		}else{
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.$billedPrice.'</td>
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0';
			$content .= '</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0</td>
		...
		$grandTotalAmountFinal += number_format($subTotal, 2);
			...
			/*trans_data field*/
			$transJsonArrayObject = (array($usdCardRows->card_number => array('product_name'=>$jsonDecodeProduct[$jsonDecodeIncrement], 'quantity'=>$jsonDecodeQuantity[$jsonDecodeIncrement], 'unit_price'=>$prideDieselPrice, 'GST'=>$gstByCard, 'PST'=>$pstByCard, 'QST'=>$qstByCard, 'amountwithouttax' => number_format($priceAfterQty, 2), 'grand_total'=> $subTotal)));
			$transationDetails[$usdCount] = $transJsonArrayObject;
			/*invoice_data field*/
			$jsonArrayObject = (array('card_number' =>$usdCardRows->card_number,'transaction_date' => $usdCardRows->transaction_date, 'transaction_id' => $usdCardRows->transaction_id));
			$arr[$usdCount] = $jsonArrayObject;
				$allGSTbyCard += $gstByCard;
				$grandGST += $gstByCard;
				$allQSTbyCard += $qstByCard;				
				$grandQST += $qstByCard;
				$allPSTbyCard += $pstByCard;
				$grandPST += $pstByCard;
		$jsonDecodeIncrement++;$usdCount++;
		}
		$transactionDetails = json_encode($transationDetails);
    ...
    <table class="table" border="" style="border: 1px solid #dcdcdc; font-size:8px; margin:20px auto; border-collapse:collapse;" cellspacing="0" cellpadding="3">  
		 ...
		 <tr>
			...
			<td>0</td>
			<td>0</td>
			<td>'.number_format($allGSTbyCard, 2).'</td>
			<td>'.number_format($allQSTbyCard, 2).'</td>
			<td>'.number_format($allPSTbyCard, 2).'</td>
			<td>'.number_format($totalDiscount, 2).'</td>
			<td>'.floor($totalTransAmount * 100)/100 .'</td>
			<td>CAD</td>
		</tr>
	...
	//}	
		$content .= '<br /><h3>Grand Totals</h3>
		 ...
		 <tr>
			...
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">0.00</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.number_format($totalDiscountFinal, 2).'</td>
			<td style="border-bottom: 1px solid #dcdcdc;border-right: 1px solid #dcdcdc;">'.floor($grandTransAmount*100)/100 .'</td>
			<td style="border-bottom: 1px solid #dcdcdc;">CAD</td>
		</tr>
		<tr><td colspan="17" style="text-align:right; border-bottom: 1px solid #dcdcdc;">Payment Recieved 0.00</td></tr>
		<tr><td colspan="17" style="text-align:right;">Amount Due '.floor($grandTransAmount*100)/100 .'</td></tr>
		 </table>
		...
		<p style="">QST# 1221749509TQ0001</p>'; 
		  ...
		  $data['invoice_data'] = $invoice_data;
		  $data['trans_data'] = $transactionDetails;		  
		  $data['grand_total'] = number_format($grandTransAmount, 2);
		  $data['date_created'] = date('Y-m-d H:i:s');
		  $data['date_modified'] = date('Y-m-d H:i:s');
		  //$this->db->insert('transaction_invoice', $data);
		  $obj_pdf->writeHTML($content);
